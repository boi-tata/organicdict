class OrganicDict(dict):
    ...
    def search(self, path):
        """Check for a path in object. If is a existent path, return the value.
    ...
    def extract(self, path):
        """Return a `OrganicDict` instance with the result of path as
        content."""
        member = self.search(path)
        assert isinstance(member, dict), "Path not exists"
        return OrganicDict(member)
